...
def multi_agent_visualize_frame(cv_frame_data, processed_frame, rays, image, agent_action):
    ...
    for box_key in ['box1', 'box2']:
        if box_key in cv_frame_data and len(cv_frame_data[box_key]) == 2:
            bx, by = cv_frame_data[box_key]
            radius = 6 if box_key == 'box1' else 10  # Different sizes for different boxes
            cv2.circle(img, (int(bx), int(by)), radius, (0, 0, 255), -1)  # Red circles
    ...
    if 'bot_pos' in cv_frame_data and 'bot_dir' in cv_frame_data:
        ...
        for idx, ray in enumerate(rays):
            ...
            rayColor = (0, 0, 255) if cv_frame_data['bot_id'] % 2 == 0 else (0, 255, 0)  
            cv2.line(img, (int(start[0]), int(start[1])), (int(hit_x), int(hit_y)),rayColor , 1)

            # Draw cross at hit point
            if not hit_info[-1]:  # If an object was hit
